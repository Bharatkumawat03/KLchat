  ...
  return (
    <div className="flex justify-center items-center h-[80vh]">
      <div className="card bg-base-200 w-96 shadow-xl gap-4 p-10">
        ...
        <button className="btn bg-primary glass text-base-300" onClick={isLogin? handleLogin : handleSignup}>{isLogin ? "Login" : "Signup"}</button>
        <p>{isLogin ? "dont have an account ? " : "have an account ? "} 
            <button className="" onClick={() => setisLogin(!isLogin)}>{isLogin? "Sign Up" : "Login"}</button>
        </p>
      </div>
    </div>
  );
